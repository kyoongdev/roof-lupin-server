/*
  ...
  - Added the required column `endAt` to the `Space` table without a default value. This is not possible if the table is not empty.
  - Added the required column `startAt` to the `Space` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `Space` ADD COLUMN `endAt` VARCHAR(2) NOT NULL,
    ADD COLUMN `startAt` VARCHAR(2) NOT NULL;
